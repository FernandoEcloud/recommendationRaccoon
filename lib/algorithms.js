...
const jaccardCoefficient = function (userId1, userId2, callback) {
  ...
  client.sinter(user1LikedSet, user2LikedSet, function (err, results1) {
    client.sinter(user1ReviewedSet, user2ReviewedSet, function (err, results2) {
      client.sinter(user1PolledSet, user2PolledSet, function (err, results3) {
        client.sinter(user1VisitedSet, user2VisitedSet, function (err, results4) {
          // retrieving a set of the users dislike incommon
          client.sinter(user1DislikedSet, user2DislikedSet, function (err, results5) {
            // retrieving a set of the users like and dislikes that they disagree on
            client.sinter(user1LikedSet, user2DislikedSet, function (err, results6) {
              // retrieving a set of the users like and dislikes that they disagree on
              client.sinter(user1DislikedSet, user2LikedSet, function (err, results7) {
                // calculating the sum of the similarities minus the sum of the disagreements
                similarity = (results1.length + results2.length + results3.length + results4.length + results5.length - results6.length - results7.length);
                // calculating the number of movies rated incommon
                ratedInCommon = (results1.length + results2.length + results3.length + results4.length + results5.length + results6.length + results7.length);
