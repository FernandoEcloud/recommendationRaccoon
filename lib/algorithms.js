...
const jaccardCoefficient = function (userId1, userId2, callback) {
  ...
  const user1ReviewedSet = Key.userReviewedSet(userId1),
    ...
    user1ScoredSet = Key.userScoredSet(userId1),
    user1WishlistedSet = Key.userWishlistedSet(userId1),
    ...
    user2ScoredSet = Key.userScoredSet(userId2),
    user2WishlistedSet = Key.userWishlistedSet(userId2);
  // retrieving a set of the users likes incommon
  client.sunion(user1LikedSet, user1ReviewedSet, user1PolledSet, user1VisitedSet, user1ScoredSet, user1WishlistedSet, function (err, unionUser1) {
    unionUser1 = unionUser1 ? unionUser1 : []
    client.sunion(user2ReviewedSet, user2LikedSet, user2PolledSet, user2VisitedSet, user2ScoredSet, user2WishlistedSet, function (err, unionUser2) {
...
exports.updateSimilarityFor = function (userId, cb) {
  ...
  client.sunion(Key.userLikedSet(userId), Key.userDislikedSet(userId), Key.userReviewedSet(userId), Key.userPolledSet(userId), Key.userVisitedSet(userId), Key.userScoredSet(userId), Key.userWishlistedSet(userId), function (err, userRatedItemIds) {
    ...
    if (userRatedItemIds.length > 0) {
      ...
      itemLikeDislikeKeys = _.map(userRatedItemIds, function (itemId, key) {
        ...
        itemScored = Key.itemScoredBySet(itemId);

        itemWishlisted = Key.itemWishlistedBySet(itemId);

        // returning an array of those keys
        return [itemLiked, itemDisliked, itemReviewed, itemPolled, itemVisited, itemScored, itemWishlisted];
      });
    }
...
exports.predictFor = function (userId, itemId) {
  ...
  const scoredBySet = Key.itemScoredBySet(itemId);
  const wishlistedBySet = Key.itemWishlistedBySet(itemId);
      ...
      exports.similaritySum(similarityZSet, reviewedBySet, function (result2) {
        exports.similaritySum(similarityZSet, polledBySet, function (result3) {
          exports.similaritySum(similarityZSet, visitedBySet, function (result4) {
            exports.similaritySum(similarityZSet, scoredBySet, function (result5) {
              exports.similaritySum(similarityZSet, wishlistedBySet, function (result6) {
                exports.similaritySum(similarityZSet, dislikedBySet, function (result7) {
                  finalSimilaritySum = (result1 + result2 + result3 + result4 + result5 + result6) - result7;
  ...
  client.zrevrange(similarityZSet, 0, config.nearestNeighbors - 1, function (err, mostSimilarUserIds) {
    ...
    client.zrange(similarityZSet, 0, config.nearestNeighbors - 1, function (err, leastSimilarUserIds) {
      ...
      _.each(mostSimilarUserIds, function (usrId, key) {
        ...
        setsToUnion.push(Key.userScoredSet(usrId));
        setsToUnion.push(Key.userWishlistedSet(usrId));
    ...
    client.scard(reviewedBySet, function (err, likedResults2) {
      client.scard(polledBySet, function (err, likedResults3) {
        client.scard(visitedBySet, function (err, likedResults4) {
          client.scard(scoredBySet, function (err, likedResults5) {
            client.scard(wishlistedBySet, function (err, likedResults6) {
              ...
              client.scard(dislikedBySet, function (err, dislikedResults) {
                ...
                if ((likedResults + likedResults2 + likedResults3 + likedResults4 + + likedResults5 + likedResults6 + dislikedResults) > 0) {
                  // set n to the sum of the total ratings for the item
                  n = likedResults + likedResults2 + likedResults3 + likedResults4 + likedResults5 + likedResults6 + dislikedResults;
                  ...
                  pOS = (likedResults + likedResults2 + likedResults3 + likedResults4 + likedResults5 + likedResults6) / parseFloat(n);
