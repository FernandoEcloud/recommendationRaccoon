...
const changeRating = function (userId, itemId, options) {
  ...
  const feelingItemSet = options.liked || options.visited || options.survey || options.reviews ?
    Key.itemLikedBySet(itemId) : Key.itemDislikedBySet(itemId);
  ...
  if (options.visited) {
    const feelingUserSet = Key.userVisitedSet(userId);
    const mostFeelingSet = Key.mostVisited();
  }

  if (options.polled) {
    const feelingUserSet = Key.userPolledSet(userId);
    const mostFeelingSet = Key.mostPolled();
  }

  if (options.reviewed) {
    const feelingUserSet = Key.userReviewedSet(userId);
    const mostFeelingSet = Key.mostReviewed();
  }
